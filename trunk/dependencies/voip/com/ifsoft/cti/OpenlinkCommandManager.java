...
/**
 * An OpenlinkCommandManager is responsible for keeping the list of available
 ...
 */
public class OpenlinkCommandManager {

    private static final String NAMESPACE = "http://jabber.org/protocol/commands";
	private OpenlinkComponent OpenlinkComponent;
     ...
     */
    private Map<String, OpenlinkCommand> commands = new ConcurrentHashMap<String, OpenlinkCommand>();
    /**
     ...
     */
    public void addCommand(OpenlinkCommand command) {
     ...
     */
    public boolean removeCommand(OpenlinkCommand command) {
     ...
     */
    public Collection<OpenlinkCommand> getCommands() {
     ...
     *         was found.
     */
    public OpenlinkCommand getCommand(String code) {
    ...
    public IQ process(IQ packet)
    {
        ...
        OpenlinkCommand command = commands.get(commandCode);
        ...
        if (sessionid == null) {
            ...
            if (command == null) {
                ...
                //sessionid = "openlink_" + StringUtils.randomString(15);
                sessionid = String.valueOf(System.currentTimeMillis() % 65536);
                ...
                if (command.getMaxStages(null) == 0) {
                	...
                	command.execute(session, childElement, iqCommand);
                    childElement.addAttribute("sessionid", sessionid);
                    childElement.addAttribute("node", commandCode);
                    childElement.addAttribute("status", OpenlinkCommand.Status.completed.name());
                }
                else {
					...
					// the allowed actions
					if (command.addNextStageInformation(session, childElement, iqCommand))
						childElement.addAttribute("status", OpenlinkCommand.Status.executing.name());
					else
						childElement.addAttribute("status", OpenlinkCommand.Status.completed.name());
                }
            }
            ...
            synchronized (sessionid.intern()) {
            	...
            	if (OpenlinkCommand.Action.cancel.name().equals(action)) {
                    ...
                    childElement.addAttribute("sessionid", sessionid);
                    childElement.addAttribute("node", commandCode);
                    childElement.addAttribute("status", OpenlinkCommand.Status.canceled.name());
                }

                // If the user didn't specify an action then follow the default execute action
                if (action == null || OpenlinkCommand.Action.execute.name().equals(action)) {
                ...
                if (!session.isValidAction(action)) {
                ...
                else if (OpenlinkCommand.Action.prev.name().equals(action)) {
                    ...
                    if (command.addPreviousStageInformation(session, childElement, iqCommand))
	                    childElement.addAttribute("status", OpenlinkCommand.Status.executing.name());
                    else
	                    childElement.addAttribute("status", OpenlinkCommand.Status.completed.name());

                }
                else if (OpenlinkCommand.Action.next.name().equals(action)) {
                ...
                else if (OpenlinkCommand.Action.complete.name().equals(action)) {
                	// Store the completed form in the session data
                    saveCompletedForm(iqCommand, session);
                    // Execute the command and return the execution result which may be a
                    // data form (i.e. report data) or a note element
	...
	public OpenlinkComponent getOpenlinkComponent() {
		return OpenlinkComponent;
	}

	public void setOpenlinkComponent(OpenlinkComponent OpenlinkComponent) {
		this.OpenlinkComponent = OpenlinkComponent;
		;
	}
