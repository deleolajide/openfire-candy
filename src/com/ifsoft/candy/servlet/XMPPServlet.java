...
import com.ifsoft.candy.plugin.BasicStreamID;
import com.ifsoft.candy.plugin.WSConnection;
...
public final class XMPPServlet extends WebSocketServlet
{
    ...
    public XMPPServlet() {
    	sockets = ( ( CandyPlugin) XMPPServer.getInstance().getPluginManager().getPlugin( "candy" ) ).getSockets();
    }
	...
	public class XMPPWebSocket implements WebSocket.OnTextMessage {
		...
		public void disconnect()
        {
            ...
            Log.debug( "Total candy created : " + sockets.size() );
            try {
            	if (conn.isOpen())
	            {
	                conn.disconnect();
	...
	private XMPPWebSocket xmppConnect(String username, String password, String resource, HttpServletRequest request)
    {
    	...
    	// make sure to pass the resource as part of the digest
 		...
 		if ( JiveGlobals.getProperty("candy.header.remoteaddr") != null && request.getHeader( JiveGlobals.getProperty("candy.header.remoteaddr") ) != null) {
 			remoteAddr = request.getHeader( JiveGlobals.getProperty("candy.header.remoteaddr") );
 		}
